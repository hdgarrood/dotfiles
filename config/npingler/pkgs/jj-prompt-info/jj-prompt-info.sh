...
if [ -n "${bookmarks:-}" ]; then
  ...
  # If there's no bookmark, find the latest common ancestor of @- and trunk()
  # and print how old it is
  parts+=("[$(jj log --ignore-working-copy -n 1 --no-graph --color=never \
    -r "fork_point(@- | trunk())" \
